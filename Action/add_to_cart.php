...
if(isset($_GET["productid"])){
  ...
  //Checks if the user has logined
    if(isset($_SESSION['user_id'])){
        ...
        if(!empty($all_cart)){
            echo ("<script>alert('Product already in the cart'); window.location.href = '../view/Allproduct.php';</script>");
        }else{
            $result = add_to_cart_controller($p_id,$ip_add,$c_id, $qty);
            if($result){
               echo ("<script>alert('Product Successfully added!'); window.location.href = '../view/Allproduct.php';</script>");
            }else{
                echo ("<script>alert('Product Not added'); window.location.href = '../view/Allproduct.php';</script>");
            }
        }
